...
context('importModal', () => {
  ...
  it('can validate new journal registration', () => {
    ...
    cy.get(`[data-testid="submit-create-journal-button"]`).click();
    cy.get(`#new-journal-title-helper-text`).should('have.text', 'Tittel er et obligatorisk felt');
    cy.get(`#new-journal-issn-helper-text`).should('have.text', 'ISSN er ikke på korrekt format (NNNN-NNNC)');
    cy.get(`[data-testid="new-journal-form-error"]`).should('have.text', 'Det er feil i tidsskrift-skjema');
