    ...
    if (inputValue !== '') {
      let temp = await axios.get(
        process.env.REACT_APP_CRISREST_GATEKEEPER_URL +
          '/institutions?cristin_institution=false&lang=nb,en&name=' +
          inputValue,
      ...
      for (let i = 0; i < temp.data.length; i++) {
        places.push({
          ...
          label: temp.data[i].institution_name.hasOwnProperty('en')
            ? temp.data[i].institution_name.nb + ' / ' + temp.data[i].institution_name.en
            : temp.data[i].institution_name.nb,
    ...
    if (props.institution.cristinInstitutionNr) {
      let temp = await axios.get(
        process.env.REACT_APP_CRISREST_GATEKEEPER_URL +
          ...
          props.institution.cristinInstitutionNr +
          '.0.0.0&per_page=900&lang=nb',
        JSON.parse(localStorage.getItem('config'))
      );
      let units = [];
      for (let i = 0; i < temp.data.length; i++) {
