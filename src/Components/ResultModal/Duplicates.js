...
export function Duplicates(props) {
  ...
  const [issn, setIssn] = React.useState(
    props.publication.hasOwnProperty('channel') && props.publication.channel.hasOwnProperty('issns')
      ? props.publication.channel.issns[0]
      : ''
  );
  const [authorChecked, setAuthorChecked] = React.useState(false);
  const [author, setAuthor] = React.useState(
    props.publication.authors[0].hasOwnProperty('authorName')
      ? props.publication.authors[0].authorName
      : props.publication.authors[0].surname + ', ' + props.publication.authors[0].first_name.subStr(0, 1)
  );
  ...
  useEffect(() => {
    async function fetch() {
      ...
      if (publication.hasOwnProperty('doi')) {
        ...
        searchString = '?title=' + title + '&published_since=' + (published - 1) + '&published_before=' + published;
  ...
  function handleChange(event) {
    ...
    event.target.value !== 'true' && event.target.value !== 'false'
      ? dispatch({
          ...
          payload: duplicate.find((element) => element.data.cristin_result_id === event.target.value).data,
        })
      : console.log();
  }
    ...
    <div>
      <ul className={`no-padding`}>
        <RadioGroup onChange={handleChange} value={state.selected}>
          ...
          {duplicate.length > 0 ? (
            duplicate.map((item, i) => <FormControlLabel key={i} control={<Result data={item.data} />} />)
          ) : (
            <p>Det finnes ingen Cristinpublikasjoner som matcher importpublikasjonen</p>
          )}
          <Button variant="contained" color="primary" hidden={isOpen} onClick={() => handleOpen()}>
            {' '}
            Søk på nytt{' '}
          </Button>
